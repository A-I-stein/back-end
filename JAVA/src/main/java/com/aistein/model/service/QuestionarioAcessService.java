...
public class QuestionarioAccessService {
    ...
    public static ArrayList<Questao> getQuestoesFromCodQuest(int codQuest){
        ...
        try{
            result = SQL.query("SELECT * FROM questao WHERE "+ COD_QUESTIONARIO
                               + " = " + codQuest);
            
            if(result.next()) {
                do{
                    Questao questao = new Questao(
                            result.getInt("Codigo_Quetionario"),
                            result.getInt("Codigo_Pergunta"), 
                            result.getString("Descricao_Pergunta"),
                            result.getString("Descricao_Resposta_Correta"),
                            result.getString("Descricao_Resposta_Incorreta_1"),
                            result.getString("Descricao_Resposta_Incorreta_2"),
                            result.getString("Descricao_Resposta_Incorreta_3"));
    ...
    public static Questionario getQuestionarioFromCodQuestionario(int 
        ...
        ArrayList<Questionario> questionarios = get("SELECT * FROM questionario"
                + " WHERE" + CODIGO_QUESTIONARIO + "=" + codQuestionario);
    ...
    public static boolean insert(Questionario questionario){
        ...
        String stm = "INSERT INTO questionario (" + CODIGO_QUESTIONARIO + ", " 
                   + MATERIA + ") VALUES ('" + questionario.getCodQuestionario()
    ...
    /**
     * Atualiza o questionário no bd correspondente ao objeto Questionario 
     * recebido
     * @param questionario
     * @return true se não houver problemas na operação.
     */
