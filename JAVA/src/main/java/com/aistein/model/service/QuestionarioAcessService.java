...
public class QuestionarioAccessService {
    ...
    /**
     * Retorna os códigos das Perguntas.
     * @return os codigos de perguntas que compõe o questionário.
     */
    public static ArrayList<Integer> getCodsPerguntas(){
        
        ArrayList<Integer> perguntas = new ArrayList<>();
        
        try{
            result = SQL.query("SELECT * FROM pergunta WHERE "+ COD_QUESTIONARIO
                               + " = " + codQuest);
            
            if(result.next()) {
                do{
                    int pergunta = result.getInt("Codigo_Resposta");
